...
/// See also:
///
///  * [NCTabBar], which displays a row of tabs.
///  * [NCTabBarView], which displays a widget for the currently selected tab.
///  * [NCTabBar.indicator], which defines the appearance of the selected tab
///    indicator relative to the tab's bounds.
enum NCTabBarIndicatorSize {
  ...
  ///
  /// This value is used to align the tab's label, typically a [NCTab]
  /// widget's text or icon, with the selected tab indicator.
  label,
}

/// A material design [NCTabBar] tab.
///
...
///  * [NCTabBar], which displays a row of tabs.
///  * [NCTabBarView], which displays a widget for the currently selected tab.
///  * [TabController], which coordinates tab selection between a [NCTabBar] and a [NCTabBarView].
///  * <https://material.io/design/components/tabs.html>
class NCTab extends StatelessWidget {
  /// Creates a material design [NCTabBar] tab.
  ///
...
class _TabLabelBar extends Row {
  _TabLabelBar({
    Key? key,
...
class _IndicatorPainter extends CustomPainter {
  ...
  final NCTabBarIndicatorSize? indicatorSize;
  ...
  Rect indicatorRect(Size tabBarSize, int tabIndex) {
    ...
    assert(tabIndex >= 0);
    assert(tabIndex <= maxTabIndex);
    double tabLeft = 0.0, tabRight = 0.0;
    ...
    /// 滑动条的宽和高
    double indicatorWidth = 18;
    double indicatorHeight = 4;

    if (indicatorSize == NCTabBarIndicatorSize.label) {
    ...
    return Rect.fromLTWH(tabLeft, tabBarSize.height - indicatorHeight, indicatorWidth, indicatorHeight);
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (controller.indexIsChanging) {
      ...
      final RRect targetRect = RRect.fromRectAndRadius(indicatorRect(size, controller.index),Radius.circular(size.height / 2));
      _currentRect = RRect.lerp(targetRect, _currentRect ?? targetRect, _indexChangeProgress(controller));
    } else {
      // The user is dragging the TabBarView's PageView left or right.
      final int currentIndex = controller.index;
      final RRect? previous = currentIndex > 0 ? RRect.fromRectAndRadius(indicatorRect(size, currentIndex - 1),Radius.circular(size.height / 2)) : null;
      final RRect middle = RRect.fromRectAndRadius(indicatorRect(size, currentIndex),Radius.circular(size.height / 2));
      final RRect? next = currentIndex < maxTabIndex ? RRect.fromRectAndRadius(indicatorRect(size, currentIndex + 1),Radius.circular(size.height / 2)) : null;
      ...
      if (value == index - 1.0)
        ...
        _currentRect = previous == null ? middle : RRect.lerp(middle, previous, index - value);
      else
        _currentRect = next == null ? middle : RRect.lerp(middle, next, value - index);
    }
    assert(_currentRect != null);

    final ImageConfiguration configuration = ImageConfiguration(
      size: _currentRect!.outerRect.size,
      textDirection: _currentTextDirection,
    );
    // _painter.paint(canvas, _currentRect.outerRect.topLeft, configuration);
    ///画线
    canvas.drawRRect(_currentRect!, Paint()..color = Color(0xFF00B88F));
  }
...
/// See also:
///
///  * [NCTabBarView], which displays page views that correspond to each tab.
class NCTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// Typically a list of two or more [NCTab] widgets.
  ///
  /// The length of this list must match the [controller]'s [TabController.length]
  /// and the length of the [NCTabBarView.children] list.
  ...
  /// and the entire [NCTabBar] is scrollable. Otherwise each tab gets an equal
  ...
  /// the indicator with the tab's text for [NCTab] widgets and all but the
  /// shortest [NCTab.text] values.
  ///
  ...
  /// is [NCTabBarIndicatorSize.tab] the tab's bounds are as wide as the space
  /// occupied by the tab in the tab bar. If [indicatorSize] is
  /// [NCTabBarIndicatorSize.label], then the tab's bounds are only as wide as
  ...
  /// tab's overall bounds if [indicatorSize] is [NCTabBarIndicatorSize.tab]
  /// (the default) or relative to the bounds of the tab's widget if
  /// [indicatorSize] is [NCTabBarIndicatorSize.label].
  ///
  ...
  /// An optional callback that's called when the [NCTabBar] is tapped.
  ///
  ...
  @override
  _NCTabBarState createState() => _NCTabBarState();
}

class _NCTabBarState extends State<NCTabBar> {
  ...
  void _initIndicatorPainter() {
    _indicatorPainter = !_controllerIsValid
        ...
        : _IndicatorPainter(
            ...
            indicatorSize: widget.indicatorSize ?? TabBarTheme.of(context).indicatorSize as NCTabBarIndicatorSize?,
            tabKeys: _tabKeys,
  ...
  @override
  void didUpdateWidget(NCTabBar oldWidget) {
  ...
  void _scrollToControllerValue() {
    ...
    if (value == index - 1.0)
      ...
      offset =
          leadingPosition == null ? middlePosition : lerpDouble(middlePosition, leadingPosition, index - value);
    else
      offset =
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> wrappedTabs = List<Widget>.filled(widget.tabs.length, Container(), growable: false);
    ...
    for (int index = 0; index < tabCount; index += 1) {
      ...
      if (!widget.isScrollable) wrappedTabs[index] = Container(child: wrappedTabs[index]);
    }
...
/// selected tab.
///
/// This widget is typically used in conjunction with a [NCTabBar].
///
/// If a [TabController] is not provided, then there must be a [DefaultTabController]
/// ancestor.
///
/// The tab controller's [TabController.length] must equal the length of the
/// [children] list and the length of the [NCTabBar.tabs] list.
///
/// To see a sample implementation, visit the [TabController] documentation.
class NCTabBarView extends StatefulWidget {
  ...
  /// One widget per tab.
  ///
  /// Its length must match the length of the [NCTabBar.tabs]
...
class _NCTabBarViewState extends State<NCTabBarView> {
  ...
  @override
  void didUpdateWidget(NCTabBarView oldWidget) {
    super.didUpdateWidget(oldWidget);
